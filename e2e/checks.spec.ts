...
test('concealment: invisible is 50% and wins over concealed when a target carries both', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('button', { name: /New battle/ }).click();
  await page.getByRole('button', { name: '+ Add' }).click();
  await page.getByPlaceholder('Gargoyle').fill('Gargoyle');
  ...
  const rows = page.locator('[data-attack]');
  await expect(rows.nth(0)).toContainText('Miss chance 50% — invisible');

  // Concealed too, now — invisible still wins (50%, not 20%).
  await page.getByRole('button', { name: /Gargoyle/ }).first().click({ button: 'right' });
  await page.getByRole('button', { name: 'Concealed', exact: true }).click();
  await page.getByRole('button', { name: 'Close', exact: true }).first().click();
  await expect(rows.nth(0)).toContainText('Miss chance 50% — invisible');
  await expect(rows.nth(0)).not.toContainText('20%');
});
