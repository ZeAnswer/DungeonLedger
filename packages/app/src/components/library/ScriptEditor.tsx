...
export function ScriptEditor({ value, onChange, errors }: { value: string; onChange: (v: string) => void; errors: ScriptError[] }) {
  ...
  useEffect(() => {
    ...
    // truncate what the user just typed. An external change (loading a different script, JSON-tab round trip)
    // always lands while this field is blurred, so this only skips genuine self-echo.
