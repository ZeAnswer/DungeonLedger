...
| `player.paramsOf(recordId).<name>` | another record's chosen tags, as a list (`[]` if that record isn't on the sheet, or is on the sheet but switched off) — for a record that must stay in sync with a choice made elsewhere, e.g. `player.paramsOf('monster-killer').types` |
